...
void RunTest(int senders_count, int receivers_count, int buff_size,
             int close_limit) {
  ...
  for (int i = 0; i < senders_count; ++i) {
    threads.emplace_back(
        [&channel, &counter, &free_slots, &send_values, &was_closed, i]() {
          for (;;) {
  ...
  for (int i = 0; i < receivers_count; ++i) {
    threads.emplace_back(
        [&channel, &free_slots, &recv_values, &was_closed, i]() {
          for (;;) {
            ++free_slots;
...
TEST(Correctness, Simple) { RunTest(1, 1, 1, 300); }

TEST(Correctness, Senders) { RunTest(4, 1, 3, 300); }

TEST(Correctness, Receivers) { RunTest(1, 6, 4, 300); }

TEST(Correctness, SmallBuf) { RunTest(8, 2, 2, 300); }

TEST(Correctness, BigBuf) { RunTest(3, 3, 100, 300); }

TEST(Correctness, Random) { RunTest(8, 8, 8, 300); }